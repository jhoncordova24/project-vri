          ...
          {newsList && newsList.length > 0 ? (
            newsList.map((item) => (
              <Link
                key={item.id}
                to={`/noticias/${item.id}`}
                className="group flex gap-3.5 items-start py-3 first:pt-0 last:pb-0 hover:bg-slate-50 -mx-2 px-2 transition-colors"
              >
                {item.imagen_url && (
                  <div className="w-20 h-14 sm:w-24 sm:h-16 flex-shrink-0 overflow-hidden bg-slate-100 border border-slate-200">
                    <img
                      src={item.imagen_url}
                      alt={item.titulo}
                      loading="lazy"
